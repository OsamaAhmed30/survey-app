...
export const ContextProvider = ({ children }) => {
  // const [currentUser, _setCurrentUser] = useState(JSON.parse(localStorage.getItem('ACCESS_USER'))|| {});
   const [currentUser, setCurrentUser] = useState({});
  const [userToken, _setUserToken] = useState(localStorage.getItem('ACCESS_TOKEN') || '');
  const [surveys, setSurveys] = useState(tmpSurveys)
  const [toast, _setToast] = useState({message : '' , show:false})
  const [questionTypes] = useState(["text" , "checkbox" , "textarea" , "select" , "radio"])
  ...
  const showToast = (toast) => {
    _setToast({message : toast , show : true})
    setTimeout(()=>{
      _setToast({message : '' , show:false})
    },5000)
